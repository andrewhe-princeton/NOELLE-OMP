...
for(int32_t t = 1; t <= tsteps;   t = t + 1){
  ...
  uint32_t call, call2, call4;
  #pragma omp parallel 
  {
  call = _ZL10num_blocksii((iter - 2), block.field0);
  call2 = _ZL10num_blocksii((iter - 2), block.field1);
  call4 = _ZL10num_blocksii((iter - 2), block.field2);
  }
  grid.field0 = call;
  grid.field1 = call2;
  grid.field2 = call4;
